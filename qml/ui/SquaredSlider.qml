...
// three `radius` values that square off the track, the tick marks and the
// knob. The fill follows `track.radius`, so squaring the track squares it too.
//
// OmniGear wants squared controls without restyling every other Omarchy panel,
// so the component is copied rather than themed in place. Re-sync this file
// when upstream PanelSlider changes; the diff against it should stay at those
// three lines plus this header, the pragma above and the `qs.Ui` import.
//
// The pragma is not upstream's. Upstream is never linted from here, and
// vendoring it brings in unqualified access to `root` and `track` from inside
// the tick Repeater's delegate; binding the delegate scope settles that.
//
// The approach — and the three radius values — follow the same vendoring done
// for WaveBar's WavebarSlider.qml.
